...
export const usePredictionStore = create((set, get) => ({
  ...
  fetchAll: async () => {
    ...
    try {
      ...
      set((s) => {
        const backendIds = new Set(data.map((p) => p.id || p._id));
        const localOnly = s.predictions.filter(
          (p) => (p.id || p._id) && !backendIds.has(p.id || p._id),
        );
  ...
  addPrediction: (pred) =>
    set((s) => ({
      ...
      selectedId: pred.id || pred._id || s.selectedId,
    })),
  ...
  removePrediction: async (id) => {
    ...
    set((s) => ({
      predictions: s.predictions.filter((p) => (p.id || p._id) !== id),
  ...
  getSelected: () => {
    ...
    if (selectedId) return predictions.find((p) => (p.id || p._id) === selectedId);
