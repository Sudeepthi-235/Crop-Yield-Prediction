...
function EmptyState() {
  const { user } = useUser();
  const email = user?.primaryEmailAddress?.emailAddress || user?.fullName || "Farmer";
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
        <h2 className="text-2xl font-display font-bold text-forest-900 mb-2">Welcome, {displayName}!</h2>
  ...
  return (
    <div className="flex-1 p-6 lg:p-8 space-y-5">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-12 h-12 bg-sage-200 rounded-2xl animate-pulse" />
        <div className="space-y-2">
          <div className="w-32 h-5 bg-sage-200 rounded animate-pulse" />
          <div className="w-20 h-3 bg-sage-200 rounded animate-pulse" />
        </div>
      </div>
      <div className="w-full h-24 bg-sage-200 rounded-2xl animate-pulse" />
      <div className="grid grid-cols-2 gap-3">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="h-20 bg-sage-200 rounded-2xl animate-pulse" />
        ))}
      </div>
      <div className="w-full h-32 bg-sage-200 rounded-2xl animate-pulse" />
    </div>
  );
}
